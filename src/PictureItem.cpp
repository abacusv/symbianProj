...
PictureItem::PictureItem(const QSizeF& size, const QPixmap& p, QObject* parent) :
    QObject(parent), QGraphicsPixmapItem(p)
{
    m_size = size;
    
    // Scale pixmap into right size
...
qreal PictureItem::rotationAngleZ() const
{
    return m_rotationAngleZ;
}

void PictureItem::setRotationAngleZ(qreal angle)
{
    if (m_rotationAngleZ != angle) {
        m_rotationAngleZ = angle;
        QPointF c = boundingRect().center();
        QTransform t;
        t.translate(c.x(), c.y());
        t.rotate(m_rotationAngleZ); // Z Axis
        t.translate(-c.x(), -c.y());
        setTransform(t);
    }
}
...
void PictureItem::setRotationAngleY(qreal angle)
{
    if (m_rotationAngleY != angle) {
        ...
        QTransform t;
        t.translate(c.x(), c.y());
        t.rotate(m_rotationAngleY, Qt::YAxis); // Y Axis
        t.translate(-c.x(), -c.y());
        setTransform(t);
    }
}
...
void PictureItem::animatePosition(const QPointF& start, const QPointF& end, int duration, const QEasingCurve& curve)
{
...
{
    // Start animate this class
    QPropertyAnimation* anim = new QPropertyAnimation(this, "rotationAngleZ");
    ...
    // easing curve
    anim->setEasingCurve(QEasingCurve::InOutBack); // Linear
